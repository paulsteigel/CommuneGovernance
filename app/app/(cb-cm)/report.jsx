...
// Tab "Số liệu" — CB_CM scope: lĩnh vực mình quản lý
// Logic giống LANH_DAO/CB_THON, khác ở scope filter
...
export default function CbCmReport() {
  ...
  // CB_CM: filter indicators by own linh_vuc_codes
  const allowedLv  = new Set(user?.linh_vuc_codes || []);
  const indicators = (manifest?.indicators || []).filter(ind =>
  ...
  function renderIndicatorCard(ind) {
    ...
    return (
      <View key={ind.chi_so_id} style={styles.indCard}>
        ...
        {hasData ? (
          <View style={styles.valRow}>
            <View style={styles.valBox}>
              ...
              <Text style={styles.valLabel}>Hiện tại ({currentYear})</Text>
            </View>
            {compareYear && (
              <View style={styles.valBox}>
                ...
                <Text style={styles.valLabel}>Kỳ trước ({compareYear})</Text>
              </View>
            )}
          </View>
        ) : (
        ...
        {/* Mini by-thon breakdown */}
        {hasData && d.by_thon && Object.keys(d.by_thon).length > 0 && (
              ...
              <View key={thon} style={styles.thonRow}>
                ...
                {c?.by_thon?.[thon] !== undefined && (
                  <Text style={styles.thonCompare}>
                    (trước: {typeof c.by_thon[thon] === "boolean"
      ...
      <View style={styles.header}>
        ...
        <View style={styles.summaryRow}>
          <View style={styles.sumItem}>
            ...
            <Text style={styles.sumLabel}>Chỉ số active</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  thonVal:     { ...TYPOGRAPHY.caption, color: COLORS.textPrimary, fontWeight: "500" },
