...
async function syncToSheets(req, res) {
  ...
  for (const xa of xaList) {
    ...
    try {
      ...
      // Query VERIFIED submissions; filter by checkpoint in memory
      // (avoids needing a composite Firestore index on status + verified_at)
      const subsSnap = await paths.submissions(xa_code)
        .where("status", "==", SUBMISSION_STATUS.VERIFIED)
        .get();

      const lastSyncMs = lastSyncAt.toMillis ? lastSyncAt.toMillis() : 0;
      const subs = subsSnap.docs
        .map(d => d.data())
        .filter(s => {
          if (!s.verified_at) return false;
          const vMs = s.verified_at.toMillis ? s.verified_at.toMillis() : 0;
          return vMs > lastSyncMs;
        });
